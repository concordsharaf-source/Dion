...
import { Bell, Home, LayoutList, Settings, Store, Users } from 'lucide-react'
 ...
 *
 * ملاحظة مهمة: كل التنسيق هنا عبر أصناف Tailwind مباشرة (بلا أصناف CSS مخصّصة)،
 * لأن أصنافًا غير معرّفة تجعل الشريط ينهار إلى عناصر متراصّة عموديًا.
 * الشبكة `grid-cols-5` هي التي تضمن توزيع التابات بالتساوي على عرض الشاشة.
 */
export function BottomNav() {
  ...
  const partiesLabel = isMerchant ? 'العملاء' : 'الديون'
        ...
        {items.map((item) => (
          <li key={item.to} className="min-w-0">
            <NavLink
              ...
              className={({ isActive }) =>
                clsx(
                  'relative flex min-h-[3.75rem] w-full flex-col items-center justify-center gap-1 px-1 text-center text-[0.6875rem] font-bold transition',
              ...
              {({ isActive }) => (
                <>
                  <span className="relative">
                    <item.icon size={22} strokeWidth={isActive ? 2.4 : 1.9} aria-hidden="true" />
