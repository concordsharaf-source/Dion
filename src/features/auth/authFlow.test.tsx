...
async function openAccountForm(user: User, role: 'عميل' | 'تاجر') {
  await user.click(await screen.findByRole('button', { name: `أنا ${role}` }, { timeout: 8000 }))
...
describe('حساب المستخدم على هذا الجهاز', () => {
  it('يطلب الاسم والهاتف وكلمة المرور وتأكيدها، مع «هل نسيت كلمة المرور»', async () => {
    ...
    // لا تُكرَّر أزرار النوع في الصفحة التالية
    expect(screen.queryByRole('button', { name: 'أنا عميل' })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'أنا تاجر' })).not.toBeInTheDocument()
  })
