...
## 5) تأكيد البريد + القوالب العربية (جاهزة — تحتاج SMTP خاصًا)

### الحالة الآن
- `mailer_autoconfirm = true` (الحساب يعمل فور التسجيل بلا انتظار رسالة) — لأن القوالب لا يمكن تعديلها
  على الخطة المجانية مع خدمة البريد الافتراضية، فتأتي الرسائل بالإنجليزية وبحدود إرسال ضيقة.
- الكود جاهز بالكامل لتشغيل التأكيد متى أردت: عند وصول الرابط يتأكد الحساب ويُنشأ ملفك الشخصي
  تلقائيًا من بيانات التسجيل (بلا إعادة كتابة الاسم أو الهاتف).

### قيد الخطة المجانية (ظهَر عند التنفيذ)
```
...
```
⇒ المطلوب **SMTP خاص** (Resend / Brevo / Gmail — كلها بحدود مجانية تكفي).
بعد إدخاله في: Authentication ← Emails ← SMTP Settings، يصبح تعديل القوالب متاحًا.

### القوالب الجاهزة
في `supabase/email-templates/` (خمسة قوالب عربية `RTL` + العناوين في `subjects.txt`) مع دليل
`README.md`، وبرنامج جاهز للتطبيق دفعة واحدة:
```bash
export SUPABASE_ACCESS_TOKEN=sbp_...        # مؤقّت — لا يُكتب في المستودع
node scripts/apply-email-templates.mjs      # يطبّق القوالب + يفعّل تأكيد البريد
```

### لماذا صيغة `token_hash` لا الصيغة الافتراضية؟
التطبيق بتوجيه الهاش (`createHashRouter`)، والصيغة الافتراضية تضع الرمز بعد `#`
(`#access_token=…`) فيتعارض مع المسار ويضيع. لذلك روابط القوالب:
`{{ .SiteURL }}/?token_hash={{ .TokenHash }}&type=email` (أو `recovery` / `email_change` / `magiclink` / `invite`).
التطبيق يقرأها في `src/app/EmailLinkGate.tsx` ويتحقق منها بـ`verifyOtp` ثم ينظّف العنوان ويفتح الشاشة المناسبة
(الاستعادة تفتح «الأمان» لتعيين كلمة مرور جديدة). ويتعامل أيضًا مع الصيغة الافتراضية `#access_token=…` احتياطًا.
